...
#include "ros_gz_example_gazebo/TrackController.hh"
...
public:
    void RegisterCollision(EntityComponentManager &_ecm,
...
public:
    ...
    /// \param[in] _msg The command.
public:
    void OnCenterOfRotation(const msgs::Vector3d &_msg);
...
public:
    ...
    /// \param[in] _beltDirection Direction of the belt (in world coords).
public:
    math::Vector3d ComputeFrictionDirection(
...
public:
    ...
    /// system.
public:
    math::Quaterniond trackOrientation;
...
public:
    common::ConnectionPtr eventConnection;
...
public:
    std::unordered_set<Entity> trackCollisions;
...
public:
    std::unordered_map<Entity, math::Pose3d> collisionsWorldPose;
...
public:
    math::Vector3d centerOfRotation{math::Vector3d::Zero * math::INF_D};
...
public:
    std::chrono::steady_clock::duration maxCommandAge{std::chrono::steady_clock::duration::max()};
...
public:
    transport::Node::Publisher odometryPub;
    /// \brief Update period calculated from <odometry_publish_frequency>.
public:
    std::chrono::steady_clock::duration odometryPubPeriod{0};
    /// \brief Last sim time the odometry was published.
public:
    std::chrono::steady_clock::duration lastOdometryPubTime{0};
};
                                ...
                                EventManager &_eventMgr)
{
    ...
    this->dataPtr->eventConnection = this->dataPtr->eventManager->Connect<events::CollectContactSurfaceProperties>(
        [this](
            const Entity &_collision1,
            const Entity &_collision2,
            const math::Vector3d &_point,
    ...
    const auto velTopic = validTopic({_sdf->Get<std::string>(
                                              "velocity_topic", kDefaultVelTopic)
                                          .first,
                                      kDefaultVelTopic});
    ...
    const auto corTopic = validTopic({_sdf->Get<std::string>(
                                              "center_of_rotation_topic", kDefaultCorTopic)
                                          .first,
                                      kDefaultCorTopic});
    if (!this->dataPtr->node.Subscribe(
    ...
    const auto odometryTopic = validTopic({_sdf->Get<std::string>(
                                                   "odometry_topic", kDefaultOdometryTopic)
                                               .first,
    ...
    this->dataPtr->trackOrientation = _sdf->Get<math::Quaterniond>(
                                              "track_orientation", math::Quaterniond::Identity)
                                          .first;
    ...
    const UpdateInfo &_info, EntityComponentManager &_ecm)
{
    ...
    if (this->dataPtr->maxCommandAge != std::chrono::steady_clock::duration::max() && (_info.simTime - lastCommandTimeCopy) > this->dataPtr->maxCommandAge)
    {
